...
from shmaplib.appdata import ApplicationConfig, Shortcut
from shmaplib.constants import (DIR_CONTENT_GENERATED, OS_MAC, OS_WINDOWS,
                                VALID_OS_NAMES)
...
class IntermediateShortcutData(object):
    ...
    This can be used as output from various shortcut document parsers 
    and can be merged together at the end.

    A serialized IntermediateShortcutData document can then be hand-edited to ensure 
    the data going exported to the web application is clean and clear.
    ...
    - "Shift + 0-9"     Range of numbers (Like having the same shortcut name on all buttons)
    ...
    class Context(object):
        ...
        def add_shortcut(self, name, win_keys, mac_keys):
            """add_shortcut Add keys to existing shortcut

            Args:
                name (_type_): _description_
                win_keys (_type_): _description_
                mac_keys (_type_): _description_
            """
    ...
    def __init__(self, app_name="", version="", default_context="", os_supported=None):
        """__init__ IntermediateShortcutData is a json format that can be
                    easily hand-edited to fix shortcut errors.
        Args:
            app_name (str, optional): 
                display name of the application (Adobe Photoshop). Defaults to "".
            version (str, optional): 
                string format of the version (eg: 2015, v1.2, v1.6a). Defaults to "".
            default_context (str, optional): 
                the name of the context that will be active by default on the site. Defaults to "".
            os_supported (list, optional): list of supported os names. Defaults to None.
        """
        ...
        if os_supported:
            assert isinstance(
                os_supported, list), "the os_supported parameter must be a list"
            assert len(
                os_supported) > 0, "the os_supported parameter cannot be empty"
            assert len([o for o in os_supported if o in VALID_OS_NAMES]) > 0, \
                "the os_supported param contains invalid os names. Valid names are: " + \
                str(VALID_OS_NAMES)
        else:
    ...
    def add_shortcut(self, context_name, shortcut_name, win_keys, mac_keys):
        """add_shortcut _summary_

        Args:
            context_name (_type_): _description_
            shortcut_name (_type_): _description_
            win_keys (_type_): _description_
            mac_keys (_type_): _description_
        """
        if context_name not in self._context_lookup:
        ...
        self._context_lookup[context_name].add_shortcut(
            shortcut_name, win_keys, mac_keys)

    def extend(self, idata):
        """Merges the data from one intermediate data object into this one"""
        assert isinstance(
            idata,
            IntermediateShortcutData), "Can only extend (merge) with IntermediateShortcutData type"

        for source_context in idata.contexts:
            for source_shortcut in source_context.shortcuts:
                self.add_shortcut(source_context.name, source_shortcut.name,
                                  source_shortcut.win_keys, source_shortcut.mac_keys)

                # Merge key contents
                if source_context.name in self._context_lookup:
                    shortcut = self._context_lookup[source_context.name].get_shortcut(
        ...
        with codecs.open(file_path, encoding='utf-8') as idata_file:
            ...
            for context_name, shortcuts in json_idata["contexts"].iteritems():
                for shortcut_name, os_keys in shortcuts.iteritems():
                    self.add_shortcut(
                        context_name, shortcut_name, os_keys[0], os_keys[1])
    ...
    def serialize(self, output_filepath):
        ...
        # Config
        json_str += f'    "name": "{0}",\n'.format(self.name)
        json_str += f'    "version": "{0}",\n'.format(self.version)
        json_str += f'    "default_context": "{0}",\n'.format(
...
class IntermediateDataExporter(object):
    """IntermediateDataExporter Exports an intermediate .json file to the 
                contents/generated directory in the correct file format.
    ...
    def __init__(self, source, explicit_numpad_mode=False):
        ...
        assert os.path.exists(
            source), f"Source file {source[0]} does not exist"
        ...
        # Get app preferences from intermediate data format
        ...
        if OS_WINDOWS in self.idata.os:
            self.data_windows = ApplicationConfig(
                self.app_name, self.app_version, OS_WINDOWS, self.default_context_name)
        if OS_MAC in self.idata.os:
            self.data_mac = ApplicationConfig(
                self.app_name, self.app_version, OS_MAC, self.default_context_name)
    ...
    def parse(self):
        """parse WINDOWS: Iterate contexts and shortcuts
        """
            ...
            for context in self.idata.contexts:
                ...
                for shortcut in context.shortcuts:
                    for s in self._parse_shortcut(shortcut.name, shortcut.win_keys):
                        context_win.add_shortcut(
                            s, True, self.explicit_numpad_mode)
            log.info("...DONE\n")
            ...
            for context in self.idata.contexts:
                ...
                for shortcut in context.shortcuts:
                    for s in self._parse_shortcut(shortcut.name, shortcut.mac_keys):
                        context_mac.add_shortcut(
                            s, True, self.explicit_numpad_mode)
            log.info("...DONE\n")

    def export(self):
        """export Exports generated shortcuts for windows or mac.
        """
        if self.data_windows:
